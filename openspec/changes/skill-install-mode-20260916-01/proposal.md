...
- **不做**孤儿技能自动清理 / 批量垃圾回收（无任何 agent 链接的技能如何清理、何时清理）。G3 含「仓库分类下的破坏性操作范围需明确」——用户主动对某个仓库技能执行的**仓库卸载**已在切片二 `skillhub-repo-display-20260917-01` 实现（删真实目录 + 所有链接）；**自动扫描并批量清理无引用技能**仍不做，**需单独开 change**。
